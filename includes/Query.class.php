...
class scwQuery {
	...
	public function parse_request_args() {
		...
		// Hack to get around an issue where location can be a post_type
		if ( !empty( $_REQUEST['booking_location'] ) ) {
			$args['location'] = absint( $_REQUEST['booking_location'] );
		}
